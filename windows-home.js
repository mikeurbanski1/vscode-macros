...
module.exports.macroCommands = {
  ...
  LogMessage: {
    no: 6,
    func: generateConsoleLogMessage,
  },
};

async function generateConsoleLogMessage() {
  ...
  const prefix = await getLoggerPrefix(
    loggerDetails.hasPrefix,
    methodName,
    document,
  ...
  const logExpr = `${loggerDetails.logUsage}(\`${logString}\`);`;
  editor
    .edit((editBuilder) => {
...
function getLoggerDetails(document) {
  console.log("getting logger details");
  let line;
  ...
  if (!foundLoggerInit) {
    console.log(
      "Could not find a line initializing a logger - assuming one is initialized with a prefix"
    );
    hasPrefix = true;
    return null;
  } else {
    hasPrefix = line.includes("prefix:");
...
function findLowestContainer(symbols, lineNum, depth = 0) {
  console.log(
    "in find lowest container, depth =",
    depth,
    "symbols.length =",
    symbols.length
  );
  let container = symbols.find((s) => {
    console.log(
      `checking symbol (kind: ${s.kind}):`,
      JSON.stringify(s, null, 2)
    );
    return (
      s.location.range.start.line <= lineNum &&
      s.location.range.end.line >= lineNum
    );
  });

  console.log(
    `container result (kind = ${container?.kind}):`,
  ...
  if (container.kind === 4) {
    // 4 = class
    container =
      findLowestContainer(container.children, lineNum, depth + 1) || container;
  }
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (typeof v === "string") {
      ...
      for (let j = 0; j < v.length; j++) {
        ...
        str += `${" ".repeat(
          (level + 1) * indentSize
        )}${processObjectForPrinting(v[j], indentSize, level + 2)}`;
      }
      str += `\n${prefix}]`;
...
function convertLineToMultiline() {
  ...
  while (true) {
    console.log(
      `Processing the object with an opening brace at index ${firstBrace}`
    );

    let newStr =
      selectedLine.substring(lastClosingBraceIndex + 1, firstBrace + 1) + "\n";
    finalStr += newStr;

    console.log("newStr:", newStr);
    console.log("finalStr:", finalStr);

    const { finalObjectString, closingBraceIndex } = parseObject(
      selectedLine,
      firstBrace,
      indentLevel + 1,
      indentSize,
...
function parseObject(line, openingBraceIndex, indentLevel, indentSize, newStr) {
  console.log(
    `Entering parseObject - openingBraceIndex: ${openingBraceIndex}, indentLevel: ${indentLevel}`
  );
  console.log(newStr);
  ...
  for (
    let charIndex = openingBraceIndex + 1;
    ...
    if (char === "{") {
      console.log(
        `Found { at index ${charIndex} (indent level ${indentLevel})`
      );
      console.log(
        `string to append before recursion (indent level ${indentLevel}):`,
        " ".repeat(indentLevel * indentSize) + currentToken.trim() + " {\n"
      );
      newStr +=
        " ".repeat(indentLevel * indentSize) + currentToken.trim() + " {\n";
      let { finalObjectString, closingBraceIndex } = parseObject(
        line,
        charIndex,
        indentLevel + 1,
        indentSize,
      ...
      console.log(
        `closingBraceIndex and finalObjectString after returning from recursion (indent level ${indentLevel}):`,
        closingBraceIndex,
        finalObjectString
      );
      ...
      console.log(
        `string to append (indent level ${indentLevel}):`,
      ...
      newStr +=
        " ".repeat(indentLevel * indentSize) + currentToken.trim() + ",\n";
      console.log(
        `string after adding this param (indent level ${indentLevel}):`,
        newStr
      );
      currentToken = "";
    } else {
...
function changeStringChar(replaceWith) {
  ...
  const newLine =
    selectedLine.substring(0, quoteStart) +
    replaceWith +
    selectedLine.substring(quoteStart + 1, quoteEnd) +
    replaceWith +
    selectedLine.substring(quoteEnd + 1);
  editor.edit((editBuilder) => {
